...
# ring definition
add_pdn_ring -grid {grid} -layers {TopMetal1 TopMetal2} -widths {5.0} -spacings {2.0} -core_offsets {4.5} -connect_to_pads
# metal strips definition for std cells 
add_pdn_stripe -grid {grid} -layer {Metal1}     -width {0.44}  -pitch {7.56} -offset {0}      -followpins -extend_to_core_ring
# vertical TopMetal1 stripes for power supply
add_pdn_stripe -grid {grid} -layer {TopMetal1}     -width {2.200} -pitch {75.6} -offset {13.600}             -extend_to_core_ring
# horizontal TopMetal2 stripes for power supply
add_pdn_stripe -grid {grid} -layer {TopMetal2}     -width {2.200} -pitch {75.6} -offset {13.600}             -extend_to_core_ring
# global net connections between Metal1 - TopMetal1 (it uses stacked via)
add_pdn_connect -grid {grid} -layers {Metal1 TopMetal1}
# global net connections between TopMetal1 - TopMetal1 
